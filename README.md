...
| 问题 | 做法 | 数字（真实） | 详见 |
|---|---|---|---|
| LLM NPC 太贵：每个 tick 都问模型 | 分层决策：规则先挡（冷却 / 限额），再加**事件驱动的门**——有人说话 / 有人新来 / 有任务才问模型，老搭档静默共处不算事件 | 调用率 **43% → 20%**（每分钟 80 → 38 次，输入 token −52%），代价是说话次数 −39%、被回应率 −2.6 个百分点；限流器压测下 `max_in_flight` 从未超上限 | [规划层与门](#规划层npc-有了日程日程顺便暴露了成本门的盲区真实数据) · [并发限流](#并发限流llm-调用的阀门顶不顶得住) |
| NPC 没人搭话时随机瞎逛，小镇没有"生活" | 规划层：每人每游戏日一份大模型按人设写的日程（地点枚举 + 归一化 + 模板兜底），没事时按日程行动 | 去过的地点 **5.3 → 2.7**，**91%** 时间在该在的地方，10/10 日程符合人设；成本不降（岗位成对，暴露了门的盲区） | [规划层](#规划层npc-有了日程日程顺便暴露了成本门的盲区真实数据) |
...
| 编造的事件被当成事实传下去 | 把"听说"当信号：按传闻记（hop=1），提示词要求保留不确定措辞、不背书、交代出处 | 26 个生成场景 × 3，LLM 裁判：保留不确定措辞 **69% → 100%**（去掉传闻本身带「可能」的 8 个场景后 61% → 98%），说得像亲眼所见 28% → 0%，交代出处 91% → 100%；5 个手写场景时是 80%→100% / 100%→100%，关键词版曾高估为 47%→80% | [传闻抑制](#传闻抑制把听说当成信号真实数据) · [裁判校验](#评测判定换成-llm-裁判校验一个结论站住了一个结论缩水了真实数据) |
...
| 正则拦不住事件类编造 | 自训 LoRA 分类器（Qwen2.5-1.5B）补正则盲区；**111 条大样本回测**暴露 v1 只学会"奇幻=编造"，按盲区补数据重训 v2；再用一份**主题事先定死**的 94 条测泛化 | 111 条：编造抓住率正则 **2%**、v1 **51%** → v2 **92%**；94 条未见过的类型：v1 47% → v2 **74%**（拦下率 56% → **91%**），误伤 2% 不变 | [大样本回测](#大样本回测111-条干净场景把-guard-v1-打回原形重训-v2真实数据) · [guard 分类器](#换个方向guard-分类器补正则的盲区真实数据) |
| guard 可能把真话判成编造 | 带依据的 NLI 核查做"证据否决"：设定能推出这句话就推翻 guard | 16 条上保险未触发；111 条上 v2 误伤的唯一一条真话（蕴含 1.00）**被它救回**，组合误伤 2% → **0%**；保险本身认出 **6/9** 真话、编造 **0** 漏过 | [带依据的核查](#带依据的核查给-guard-上一道误判真事的保险真实数据) |
| 关键词判定漏判/误判，数字可信度存疑 | LLM-as-judge：强一档的模型按标签定义打标，两套判定并列 + Cohen's κ，分歧单独列出 | 幻觉累积 κ=0.66 结论不变；传闻抑制 κ=0.22，关键词漏判把提升放大，按裁判重算后修正 | [裁判校验](#评测判定换成-llm-裁判校验一个结论站住了一个结论缩水了真实数据) |
...
| 成本控制 | 分层决策：事件驱动的门——有人说话 / 有人新来 / 有任务 / 隔 90 秒主动一次才调用大模型（老搭档静默共处不算事件）；说话冷却 6s、30s 内最多 3 句、聊够后 `end_conversation` 走开；其余时间零成本规则；429 按服务端等待时间退避重试 |
| 日程 | 每个 NPC 每个游戏日一份大模型按人设写的日程（`planner.py`），没事时按日程行动而不是随机闲逛；游戏时钟默认 600 秒一天 |
...
- 重复率、编造率是关键词/正则启发式，有误判也有漏判，所以每次评测都会导出**被标记的句子**（`evals/results/*-flagged.md`）供人工核对。幻觉累积和传闻抑制两个评测已经接上 LLM 裁判做第二套判定（见"评测判定换成 LLM 裁判校验"一节）：一个结论站住了，一个结论被修正——其它仍只有关键词判定的评测，同样的偏差可能存在。
...
- 15 条一个配置，样本小；判定是关键词启发式，逐条回答在 `evals/results/*-hearsay.md`。**这张表的数字后来被 LLM 裁判校验过、又扩到 26 个生成场景——关键词漏判把提升放大了，5 个手写场景又把它低估了，真实幅度见下面"评测判定换成 LLM 裁判校验"一节，读数以那里的 26 场景裁判版为准。**
- 这一层跟 guard 分类器是互补的：guard 拦"说出口之前"，这一层管"已经说出口、被别人听到之后"不要越传越像真的。

### 评测判定换成 LLM 裁判校验：一个结论站住了，一个结论缩水了（真实数据）

README 里反复出现的一句话是"判定是关键词启发式，有误判也有漏判"——至少三次记录了关键词列表漏掉某种说法、补上之后数字变了。这次把行业里的标准做法接上：`evals/judge.py` 让一个比 NPC 强一档的模型（NPC 用 gpt-4o-mini，裁判用 gpt-4o，用同一个模型自己判自己偏差会往一个方向倒）按标签定义给每条回复打标签、附一句理由，输出走 tool calling 的枚举，不解析自由文本。各评测脚本加 `--judge` 后两套判定并列，附一致率和 Cohen's κ（把碰巧一致扣掉），**不一致的那些单独列出来——那才是该人工看的**。

**幻觉累积（`hallucination_recall --judge`，4 组场景 × 12 次，n=96）**：
...
一致率 78%，κ = 0.66。结论站住了：结构性修复的效果在两套判定下几乎一样。21 条不一致的两边各有对错——Bob 的「嗯，还没定下来」关键词判含糊、裁判判顺着编，裁判对，这句默认了国王那件事；Carol 的「哇，沙漠里的金币！你打算怎么花呢？」关键词判顺着编、裁判判含糊，这条裁判更可能是错的，她把金币当真事只是把问题推回去。两边误差方向相反，总数抵消。

**传闻抑制（`hearsay --judge`，5 场景 × 3 次，每档 n=30）**：

| 配置 | 保留不确定措辞 | 说得像亲眼所见 | 替传闻背书 | 交代得出出处 | 含糊带过 |
|---|---|---|---|---|---|
| 当成亲历事实（关键词） | 47% | 53% | 0% | 80% | 20% |
| 当成亲历事实（裁判） | **80%** | **20%** | 0% | **100%** | **0%** |
| 标成传闻（关键词） | 93% | 7% | 0% | 100% | 0% |
| 标成传闻（裁判） | **100%** | **0%** | 0% | **100%** | **0%** |

复述型 κ 只有 0.22，出处型 0.47——**关键词判定在这组上不可靠，而且错的方向是系统性的**：「听 Greta 说的，Bob 确实打算搬走」被判成"说得像亲眼所见"，因为 HEDGE 列表只有连着写的"听说"，中间隔一个人名就漏；「这是听来的，我没亲眼见着」被判成"含糊带过"，因为 UNSURE_SOURCE 里只有"也是听来的"，少一个"也"字就漏。这两处漏判都发生在**基线**那一侧（基线的回复本来就常带"听 X 说"），于是把基线压低、把提升放大。按裁判重算：保留不确定措辞是 **80% → 100%**，不是之前写的 47% → 80%；交代出处是 **100% → 100%**，之前写的 80% → 100% 完全是关键词漏判制造的。机制仍然有效果（把剩下 20% 的"说得像亲眼所见"压到 0），但幅度只有原来的一小半，出处那一项没有提升。上一节的表保留作对照，读数以这里的裁判版为准。

- 没有回头去补 HEDGE / UNSURE_SOURCE 的关键词：看了裁判结果再改关键词，就是对着这批样本过拟合，两套判定要保持独立才有互相校验的意义。
- 裁判也不是真理：上面 Carol 那条就是它判错的例子，还有它把「应该不会出大事」算成不确定措辞，偏宽。裁判的价值是给出第二个独立的、带理由的判定，把分歧暴露出来，不是替代人工。
- 裁判每条一次调用，成本跟被测调用同量级；目前只接了这两个评测，其它评测（探测题、对话风格、反思质量）仍是关键词，同样的偏差可能存在。

#### 传闻抑制扩到 26 个场景：缩水的结论又长回来一半，κ 从 0.22 到 0.47

上面 5 个场景是手写的。跟 guard 一样，让 gpt-4o 按设定出了 30 个传闻场景（`evals/gen_scenarios.py hearsay`），人工过：删 4 条"传闻"其实是设定里真有的事（水轮响、麦子减产、铁矿不多——真事没有什么可抑制的），改 3 条出处型追问（「可信吗」不是在问是谁说的），15 条以「听说 / 有人说」开头的传闻统一去掉开头——传闻是以讲述者口吻种进记忆的，讲述者自己带「听说」，NPC 照抄一遍关键词就白送分，跟手写 5 条的平铺直叙口径不一致。留 26，`hearsay --judge --scenarios`，× 3 次，每档 n≈78：

| 配置 | 保留不确定措辞 | 说得像亲眼所见 | 替传闻背书 | 交代得出出处 | 含糊带过 |
|---|---|---|---|---|---|
| 当成亲历事实（关键词） | 49% | 49% | 3% | 82% | 18% |
| 当成亲历事实（裁判） | **69%** | **28%** | 3% | **91%** | **9%** |
| 标成传闻（关键词） | 92% | 8% | 0% | 99% | 1% |
| 标成传闻（裁判） | **100%** | **0%** | 0% | **100%** | **0%** |

复述型 κ = 0.47（n=155），出处型 0.64（n=154）。

- **样本大了，提升比 5 场景的裁判版大、比关键词版小**：保留不确定措辞按裁判 69% → 100%（5 场景时是 80% → 100%），交代出处 91% → 100%（5 场景时 100% → 100%"无提升"，现在有 9 个点）。5 个场景的基线碰巧偏好，把效果低估了；关键词版 49% → 92% 仍然高估。两头都测过，真值在中间偏裁判这边。
- **过题时漏了一个口径**：15 条「听说」去掉了，但 8 个场景的传闻本身带「可能 / 考虑」（"Elsa 可能要把磨坊卖掉"），NPC 照抄「可能」裁判就算保留措辞。拆开看：带「可能」的 8 个场景基线裁判 88%，平铺直叙的 18 个场景基线 **61%** → 98%。所以干净的读数是 **61% → 98%**，+37 个点，比表里的 +31 还大——这个漏洞把基线抬高了，是往低估效果的方向错，如实记，下次生成时把「可能」也归一化掉。
- **κ 从 0.22 到 0.47 不是关键词变准了，是样本大了碰巧的一致被扣得更准**。31 条分歧里关键词错的方向还是老三样：「Elsa 刚说可能会缺」被判 asserted（HEDGE 里没有"可能"）、「这是听来的，我没亲眼见着」被判 vague（少个"也"字）、「听说确实有荆棘」被判 corroborated（"确实"在这儿是附和不是背书）。裁判自己偏宽的地方也稳定：把「Bob 刚才提到了这件事」算 hedged（交代来源 ≠ 不确定），6 条是这一类。还是没有回头改关键词表，理由同上。
...
### 带依据的核查：给 guard 上一道"误判真事"的保险（真实数据）

上一节的 guard 分类器是"看一句话判它是不是编造"，手里没有依据——训练数据当时把"镇长"当成不存在的人物，后来设定加了"镇长会巡视广场"，它理论上可能把真话判成编造。而判错的后果比编造更糟：下一节的结构性修复会拿 guard 的结论让 NPC"承认自己说错了"，等于让它否认一件真事。这条风险在已知局限里挂了很久。

2024～2026 年事实核查的主流形式换了个问法（MiniCheck、RAGAS faithfulness 都是这一路）：给两段文本——一份依据和一句陈述——问"依据能不能推出这句话"，也就是 NLI 蕴含。依据里有镇长它就不会判假，设定改了自动跟上，不用重训。`townmind/grounding.py` 用的是多语言 NLI 模型（MiniCheck 系列只有英文），依据 = 地点设定 + 镇上的事 + 人设 + 这个 NPC 的记忆，每条单独作 premise 跟台词配对、取最高蕴含概率。

它在系统里**只做"证据否决"，不单独把关**：只在 guard 说"编造"时出面，蕴含 ≥ 0.7 就推翻 guard。原因在数据里——NPC 台词大多是寒暄，跟设定是中立关系不是蕴含，单看蕴含分会把"要不要尝尝法棍"也当成没依据。

`evals/guard_fabrication.py` 扩到 16 条（原 11 条 + 5 条"真话但换了说法"，专测误判）：

| 方法 | 准确率（16 条） |
|---|---|
| 正则规则 | 62% |
| guard 分类器（LoRA，guard-v1） | 88% |
| 带依据的 NLI 单独把关 | 81% |
| guard + 证据否决（系统里实际的组合） | 88% |

- **担心的误判没有发生**：5 条换了说法的真话 guard 全判 ok，9 条 ok 样本 9/9。所以证据否决一次都没被触发，组合结果就是 guard 自己的 88%——这一层现在是保险，没赔付过。这是好消息，但要老实说：它没有带来可测的提升。
- **保险本身能覆盖多少**：`evals/grounding_probe.py` 单独测 9 条真话，两种喂法 × 两个模型。依据拼成一段喂 0/9（相关的那句被淹没），逐条喂才行；mDeBERTa-base（2.8 亿参数）认出 4/9，XLM-R large（5.6 亿）认出 6/9，编造 3 条全 0.00 没漏。剩下认不出的三条，一条是寒暄，两条是带"小尾巴"的句子（「广场那口老井三十年没干过，**镇上人都知道**」蕴含 0.00）——NLI 的定义是整句被推出，多一个设定里没有的从句就算不蕴含。这是方法的边界，换更大的模型也不会变。所以就算 guard 哪天真误判了，这层保险大约能救回三分之二。
- guard 自己的两条错也顺带看清了：「魔法学院聘书」判成 out_of_character 而不是 fabricated——标签错，但系统里照样拦下；「提拉米苏」guard 漏了、正则抓住，"任一判编造就拦"仍然成立。
- 16 条样本，数字看量级。默认关（`TOWNMIND_USE_GROUNDING`，要跟 guard 一起开），模型约 2.2GB。

### 大样本回测：111 条干净场景把 guard-v1 打回原形，重训 v2（真实数据）

上面两节的 16 条样本是手写的，写的人知道 guard 强在哪——这是评测里最常见的自欺。这次换成
`evals/gen_scenarios.py`：让 gpt-4o 按 10 个 NPC 人设 + 真实设定分批生成，每条带期望标签和
一句理由，去重后人工逐条过（`reviewed: true`，删掉了标签站不住的），得到 `evals/scenarios/guard.json`
111 条：53 条编造 + 58 条真话。同一个脚本加了 `--scenarios` / `--adapter`，跑同一套四种方法：

| 方法 | 准确率（111 条） | 编造抓住率（53 条） | 真话误伤率（58 条） |
|---|---|---|---|
| 正则规则 | 53% | 2% | 0% |
| guard-v1（16 条手写样本上 88%） | 77% | **51%** | 0% |
| 带依据的 NLI 单独把关 | 73% | 100% | 52% |
| **guard-v2**（补数据重训） | 95% | **92%** | 2% |
| **guard-v2 + 证据否决**（系统里实际的组合） | **96%** | **92%** | **0%** |

- **v1 在干净样本上只抓住一半编造。** 把 26 条漏掉的排开看，规律很明显：抓得准的是"王后御厨"
  "海盗长约翰""魔法师急着要刀"——奇幻、显眼；漏掉的是"面包师公会又提倡用优质面粉了""今早的
  肉桂卷加了点香橙粉""邻镇的校长每月来买松饼""上周我被邀请去邻镇吃晚宴"——平淡、像日常。
  它学会的是**"奇幻 = 编造"**，不是**"设定里没有 = 编造"**。回头看训练数据（`guard/domains/townmind.py`
  的主题列表），编造的例子全是"没有的地点/节日/人物"这种写出来天然显眼的类别，从没教过它
  "一个听起来很正常的行会名也是编的"。手写的 16 条恰好也都是显眼的编造，所以 88% 是被样本
  惯出来的数字。
- **修法是补数据，不是改模型。** 按这四类盲区加了 4 个编造主题（平淡的行会名 / 像正常原料的
  新东西 / 邻镇的普通人 / 自己经历过的设定外小事），配了 2 个 ok 主题（换说法复述设定里真有
  的事 / 不涉及任何设定外事物的无害琐事）——两边措辞一样平淡，模型只能靠"背景资料里有没有"来
  分，这正是要它学的边界。人设从 3 个扩到 5 个。生成时用 6 字 shingle 把跟 111 条重叠的句子
  过滤掉（`generate_data.py --exclude`），保证回测集干净。884 训练 / 157 验证，LoRA r=16、
  lr 2e-4、3 轮，RTX 5080 上 41 分钟，eval loss 0.037（v1 约 0.05）。
- **v2 抓住率 51% → 92%，代价是误伤 0% → 2%。** 漏掉的 4 条里 3 条还是平淡行会名（"旅行商会"
  "磨坊公会"判 ok，"铁匠联盟"判成 out_of_character——标签错但系统里照样拦下），说明这一类
  是硬骨头，补了数据也只是从全漏变成大部分抓住；1 条是"香橙粉"。误伤的那 1 条是
  「听说镇上到现在都没个医馆，看病还得跑邻镇」——设定里真有"没医馆"这条，v2 大概被"邻镇"
  带偏了。
- **证据否决第一次赔付。** 那条误伤，NLI 对设定的蕴含分是 1.00，被推翻，组合结果回到 0% 误伤。
  上一节说这层保险"从没触发过、没有可测收益"，现在有了：模型越严，这层保险越有用——这是
  两层叠加设计第一次在数据上体现出分工。
- 要老实说的一点：v2 的四类新主题是**看着 111 条的漏判写出来的**。句子本身没泄漏（shingle
  过滤），但类别分布是照着回测集补的，所以 92% 是"修了已知盲区"的数字，不是"泛化到未知盲区"
  的数字。后者单独测，见下一小节。

#### 泛化回测：主题事先定死的第二份场景库，v2 拦下 91%、标签对 74%

为回答"没见过的编造类型还行吗"，又生成了一份 `evals/scenarios/guard-holdout.json`（94 条）。
四类编造主题在看 v2 结果**之前**定死，跟第一份的四类、跟 v2 补的四个训练主题都不重叠：镇上的
规矩/规定、小镇历史、别的 NPC 的私事、未来安排/承诺；ok 那边除了真话换说法，加了 20 条不含任何
可核对事实的纯寒暄，测模型变严之后会不会误伤闲聊。过题时只删标签或句子站不住的 6 条（带出处的
转述、NPC 用第三人称说自己、时态混乱、纯粹自己的打算），没拿模型跑过一遍再挑。

表里多了一列**拦下率**：guard 是四分类，把编造判成 out_of_character / unsafe 标签是错的，但系统里
照样退回兜底、玩家看不到那句话——抓住率和拦下率的差就是"拦对了但标签错"的那部分。

| 94 条，主题未见过 | 编造抓住率（34） | 编造拦下率 | 真话误伤率（60） |
|---|---|---|---|
| 正则 | 0% | 0% | 0% |
| guard-v1 | 47% | 56% | 2% |
| **guard-v2** | **74%** | **91%** | 2% |
...
| 出题类别 | 条数 | v1 判对 | v2 判对 |
|---|---|---|---|
| 编造的规矩/规定 | 10 | 30% | 50%（另 5 条判成 out_of_character，全拦下） |
| 编造的小镇历史 | 9 | 44% | 67% |
| 编造别的 NPC 的私事 | 10 | 70% | **100%** |
| 编造的未来安排/承诺 | 5 | 40% | 80% |
| 真话换说法 | 40 | 98% | 100% |
| 纯寒暄 | 20 | 100% | 95% |

- **v2 的提升不只是修了已知洞。** 四类主题它一类都没专门练过，抓住率仍比 v1 高 27 个点、拦下率
  高 35 个点；真话和寒暄没有因为变严而多误伤。所以"补平淡的编造"教会它的是一个更一般的边界，
  不只是那四类。
- **但 74% 没到 92%，差距就是泛化的代价**，而且弱点很具体：规矩类 10 条里 5 条被判成
  out_of_character——"杂货铺不退货""学徒不能单独用铁砧""货郎不能在同一条街叫卖两次"，模型
  觉得这是 NPC 在说奇怪的话，而不是在编设定里没有的规则。系统层面它们都被拦下了，但标签错说明
  模型并没有"规则也可以是编的"这个概念——训练数据里没有一条是编规矩的。历史类漏的 3 条
  （"铁匠铺是我祖父建的""水轮最早是我叔叔修的"）都是真设定 + 编造的亲属，跟 guard/README 里 v1
  那次"真假混在一句里就被真的那半带偏"是同一个毛病。
- **证据否决第一次帮倒忙**（v1 那张表）：「货郎不能在同一条街叫卖两次」v1 判对了，NLI 对着"走街
  串巷的货郎"这条人设给了 0.99 的蕴含，把它推翻成 ok。NLI 把"货郎"这个词对上就算蕴含了，它不
  懂后半句是条新规则。上一节说这层保险认不出带小尾巴的真话，这里是反面：它也可能放过带真名词的
  编造。两次都是 NLI 拿词对齐当推理的边界。v2 那条判成 out_of_character，否决不触发，没暴露。
- 结论：v2 在未知类型上拦下 91%，作为"正则之后再补一道"的第二层够用；作为独立判定还不够，
  尤其是规矩类和真假混句。下一版数据该补的是这两类——这次是先测出来再补，跟 v1→v2 的顺序一样。
- 训练本身也踩了一个坑，记下来：第一次跑 346 秒/步，看起来像 GPU 没用上；`nvidia-smi` 和
  matmul 基准都正常（sm_120、35 TFLOPS），最后是关掉 Unity 和 Docker Desktop 之后掉到
  14.6 秒/步——笔记本上 GPU 显存和功耗被别的进程占着，训练脚本本身没问题。
- `guard_model.DEFAULT_ADAPTER_DIR` 已指到 `guard-v2`；`TOWNMIND_GUARD_MODEL_DIR` 可以换回 v1 对比。
...
### 规划层：NPC 有了日程，日程顺便暴露了成本门的盲区（真实数据）

Generative Agents 论文里 NPC 有三大件——记忆、反思、规划。前两件项目里早就有了，规划一直没有：NPC 没人搭话时是随机闲逛（35% 发呆，其余随机去一个地点），Alice 会在半夜跑去铁匠铺，小镇只有"碰面"没有"生活"。

`townmind/planner.py`：一个游戏时钟（`TOWNMIND_DAY_SECONDS`，默认 600 秒一天，开服定为早上 7 点），每个 NPC 每个游戏日一份日程——4～8 段"几点到几点、在哪、干什么"，由大模型按人设写一次（每人每游戏日一次调用，10 个 NPC 每 10 分钟 10 次，相对对话调用可以忽略），写不出来用按工作地点拼的模板。接进决策的只有一处：没事发生时不再随机闲逛，而是"该在的地方不在就走过去，在了就待着"，零成本；提示词多一句"现在是上午 09:00，按你今天的日程这会儿你在磨坊磨面粉"。论文里把一天递归细化到小时、再到十分钟，这里一层够了；对话打断日程之后也不重新规划，聊完自然回到当前时段该在的地方。

模型写的日程（`evals/plan_samples.py`，10 份全部过校验，归一化一段都没用上）：

- **Bob**（铁匠铺）：8-12 点在铁匠铺锻造武器和铠甲；12-13 点在广场吃午饭；13-18 点在铁匠铺继续冶炼金属；18-20 点在酒馆喝酒放松；20-24 点在铁匠铺清理工作区域
- **Jonas**（守夜人）：0-8 点在广场巡逻，确保小镇安全；8-12 点在广场检查镇上设施和秩序；12-14 点在酒馆与镇民分享午餐；14-20 点在广场继续巡逻；20-24 点在广场整理记录
- **Milo**（货郎，无固定地点）：8-12 点在杂货铺摆摊售卖新货品；12-13 点在广场休息吃午饭；13-18 点在铁匠铺帮铁匠搬运货物；18-20 点在酒馆跟村民聊天喝酒；20-24 点在磨坊聊八卦
- **Elsa**（磨坊）：……18-20 点在酒馆与镇民喝酒聊天，**发发牢骚**……

两个顺手踩到的坑，都是真实跑出来的：

- **tool schema 里的枚举对模型只是提示，不是约束。** 第一次真实跑，10 份日程里 4 份把 `place` 写成「家」「家中」「家里」「待在工作地点」——schema 里明明是六个地点的 `Literal` 枚举。之前项目里所有枚举都靠返回后再校验兜住，这次校验也兜住了（退回模板），但退回模板就浪费了那次调用。修法两层：提示词明说"没有家这个地点，你的家就是工作地点"；校验前先归一化（`coerce_places`：家一类的词映射到该 NPC 的工作地点，认不出的段丢掉，没排到的时段本来就按工作地点算）。改完 10/10 一段都不用修。
- **OpenAI 429。** 无头仿真用虚拟时钟，5 分钟的小镇几十秒就跑完，请求全挤在一起，撞到每分钟 20 万 token 的上限，撞到就退回行为树台词、污染评测的重复率。加了按服务端给的等待时间退避重试（最多 3 次），失败率从 3.5% 归零。真实服务 NPC 一多也会撞，本来就该有。

**评测**：`evals/run.py` 加 `plans` 配置和四项"过日子"指标。5 分钟 × 3 seed，真实模型：

| 指标 | full | plans | gate | plans_gate |
|---|---|---|---|---|
| 每分钟调用次数 | 80.0 | 75.2 | **37.7** | **25.9** |
| 输入 token（5 分钟） | 73.5 万 | 71.4 万 | 35.2 万 | **24.8 万** |
| 说话次数 | 215 | 195 | 132 | 74 |
| 被回应率 | 90.1% | 93.4% | 87.5% | 85.5% |
| 重复率 | 3.1% | 3.2% | 1.0% | 0.5% |
| 编造率 | 0.3% | 0.9% | 0.5% | 0.5% |
| 决策时附近平均几个人 | 2.05 | 2.38 | 2.31 | 2.70 |
| 每个 NPC 去过几个不同地点 | 5.3 | **2.7** | 5.9 | **2.8** |
| 按日程行动中「已在该在的地方」占比 | – | **91.0%** | – | **94.2%** |

先看前两列。日程做到了它该做的：去过的地点 5.3 → 2.7，91% 的时间在该在的地方，三个 seed 都稳定；对话指标持平（被回应率 90% → 93%）。**但成本没降**（80 → 75，在噪声里），附近平均几个人反而从 2.05 涨到 2.38。原因一想就明白：这个小镇的岗位是成对的——Alice 和 Iris 都在面包店，Bob 和 Greta 都在铁匠铺，按日程上班等于把搭档锁在一起一整天；而成本门的规则是"附近有人就问模型"，搭档永远在旁边，门就永远开着，哪怕模型问完选的是"待着"。**门判断的是有没有人，不是有没有事发生。** 这是规划层这一轮最值钱的发现——它不是"加了个功能"，是一个功能的数据揭示了另一个机制的缺陷。

**事件驱动的门**（`event_gate`）就是冲着这个来的：只有有人说话、有人新来（上次见到是 60 秒以前）、有任务、或隔了 90 秒没搭话了主动一次，才问模型；老搭档静默共处不算事件。后两列是结果：单独开门调用砍 53%（80 → 38），两个一起砍 68%（80 → 26），输入 token 从 73 万降到 25 万；按调用率算，full 43%，gate **20%**。代价要老实说：说话次数 215 → 132 → 74，被回应率 90% → 87.5% → 85.5%——省钱是靠少说话省的，不是同样多的话花更少的钱。但每句话的质量没掉：重复率 3.1% → 0.5%，编造率持平，砍掉的是"旁边站着个老搭档我该干嘛"那种废话。主动道别从 19 次掉到接近 0 是个副作用：对话变短了，还没聊到该道别就自然散了；压缩和关系更新走"散场"那条路，不受影响。

两个都做成开关：`event_gate` 默认开（成本砍一半、对话质量不掉，修的是真实缺陷）；`use_plans` 代码里默认关作为消融基线，`.env.example` 推荐开。实验台上都能当场切。

- 引用真实设定率在 plans_gate 那列是 29.5%（区间 16–47），比别的列低——那一列只剩 74 句话，样本太小，不当结论。
- 岗位成对是这张地图的特点；换一张岗位不重叠的地图，日程可能真的会分散人、省下调用。没测。
- 输入 token 每次调用约 1800——设定、记忆、人设全塞进提示词——这是比"问几次"更大的成本项，没动。
...
uv run python -m evals.grounding_probe                                # 带依据的 NLI 核查：喂法 × 模型大小
...
uv run python -m evals.hearsay --llm real --repeats 3 --judge         # 传闻抑制；--judge 再让 LLM 裁判判一遍并给出 κ
uv run python -m evals.hallucination_recall --llm real --repeats 12 --judge  # 幻觉累积，同上
uv run python -m evals.scale_test && uv run python -m evals.concurrency_test  # 空间索引 / 并发限流压测
uv run python -m evals.run --llm real --minutes 5 --seeds 1,2,3 --configs full,plans,gate,plans_gate  # 规划层 × 事件驱动的门
uv run python -m evals.plan_samples                                   # 让模型给每个 NPC 写一份日程，看符不符合人设
```
...
- 事件驱动的门省下的一半调用是靠 NPC 少说话省的（说话次数 −39%）；小镇明显安静了，对"看热闹"的 demo 是代价。门的两个常数（新来 60 秒、主动 90 秒）没有调过参。
- 规划层只有一层日程，没有论文里的递归细化，也没有"被打断后重新规划"；日程质量只人工看了 10 份，没有量化评测。
- 每次调用输入约 1800 token（设定、记忆、人设全塞进提示词），是比调用次数更大的成本项，没动。
- 实验台那些数字（调用率、说出口率、传到几人几手）来自单次 3 分钟的真实运行，不是多 seed 均值；四轮返工每轮都清了数据重跑，但样本量就是这个量级。
- LLM 裁判只接了两个评测；探测题、对话风格、反思质量、guard 相关的评测仍是关键词/正则判定，传闻抑制那次校验证明这类判定可以把提升放大一倍，其它表的数字要按这个折扣读。
- 大样本场景库做了 guard 两份（111 + 94 条）和传闻一份（26 场景）；幻觉累积、探测题仍是几条手写场景，同样的"手写样本惯出来的数字"风险还没排除。传闻那份过题时漏了「可能」这个口径，8/26 的场景基线被抬高，效果是低估不是高估。
...
- 幻觉累积这条链路，目前有两层真实验证过的缓解，组合起来算是走通了：前门——正则测不到的
  ...
  （见"结构性修复"小节）。前门那个 91% 后来被 111 条干净样本打回原形：guard-v1 只抓住 51%
  的编造，学会的是"奇幻=编造"而不是"设定里没有=编造"；按盲区补数据重训的 v2 抓住 92%，但
  第二份主题事先定死的 94 条上 v2 抓住 74%、拦下 91%（v1 47% / 56%）——泛化有，但规矩类
  一半被判成 out_of_character、真假混句仍漏（见"大样本回测"小节）。"guard 把真实设定误判
  成编造"：v2 误伤 1/58，被证据否决（带依据的 NLI 核查）救回，这层保险第一次赔付；它自己只能
  认出约三分之二的真话——带"小尾巴"的口语句子（"……，镇上人都知道"）NLI 判不蕴含，这是方法
  的边界（见"带依据的核查"小节）。
