...
| 编造的事件被当成事实传下去 | 把"听说"当信号：按传闻记（hop=1），提示词要求保留不确定措辞、不背书、交代出处 | LLM 裁判判定：保留不确定措辞 **80% → 100%**，说得像亲眼所见 **20% → 0%**；交代出处 100% → 100% 无变化（关键词版曾高估为 47%→80% / 80%→100%） | [传闻抑制](#传闻抑制把听说当成信号真实数据) · [裁判校验](#评测判定换成-llm-裁判校验一个结论站住了一个结论缩水了真实数据) |
...
- 15 条一个配置，样本小；判定是关键词启发式，逐条回答在 `evals/results/*-hearsay.md`。**这张表的数字后来被 LLM 裁判校验过，关键词漏判把提升放大了——真实幅度见下面"评测判定换成 LLM 裁判校验"一节，读数以那里为准。**
...
- 大样本场景库目前只做了 guard 这两份（111 + 94 条）；传闻抑制、幻觉累积、探测题仍是十几条手写场景，同样的"手写样本惯出来的数字"风险还没排除。
