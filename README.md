...
| 正则拦不住事件类编造 | 自训 LoRA 分类器（Qwen2.5-1.5B）补正则盲区 | 11 条样本准确率 **55% → 91%**（扩到 16 条：62% → 88%） | [guard 分类器](#换个方向guard-分类器补正则的盲区真实数据) |
| guard 可能把真话判成编造 | 带依据的 NLI 核查做"证据否决"：设定能推出这句话就推翻 guard | 5 条真话换说法 guard **未误判**、保险未触发；保险本身认出 **6/9** 真话、编造 **0** 漏过 | [带依据的核查](#带依据的核查给-guard-上一道误判真事的保险真实数据) |
...
- 幻觉累积这条链路，目前有两层真实验证过的缓解，组合起来算是走通了：前门——正则测不到的
  ...
  （见"结构性修复"小节）。"guard 把真实设定误判成编造"这条风险现在测过了：5 条换了说法的
  真话 guard 没有误判；加的证据否决（带依据的 NLI 核查）作为保险从没被触发过，而且它自己
  只能认出约三分之二的真话——带"小尾巴"的口语句子（"……，镇上人都知道"）NLI 判不蕴含，
  这是方法的边界（见"带依据的核查"小节）。
