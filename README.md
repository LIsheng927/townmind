...
- 幻觉累积本身（编造内容一旦被记进记忆，追问时会不会顺着编）仍未解决：最早的修复只让 NPC 少一点"自信满满地继续编"；这次又尝试把提示词从"可以纠正"改成"必须纠正 + 给例句"，真实数据显示没有效果（见上面"这次尝试"小节），"主动纠正"依然接近 0%，在提示词措辞上加码这条路大概率已经到头。不过前门这一侧有真实进展：正则测不到的事件/关系类编造（"国王要收你做徒弟"这种），已训练好但从没验证过的 guard 分类器真实数据测出 91% 准确率，能补上正则的盲区（见"换个方向"小节）——这降低的是这类编造第一次就能说出口、存进记忆的概率，是从源头减少问题，不是解决了"已经记住之后怎么纠正"这件事本身。
