...
| 正则拦不住事件类编造 | 自训 LoRA 分类器（Qwen2.5-1.5B）补正则盲区；**111 条大样本回测**暴露 v1 只学会"奇幻=编造"，按盲区补数据重训 v2 | 16 条手写样本 62% → 88%；111 条干净样本编造抓住率正则 **2%**、v1 **51%** → v2 **92%**，误伤 0% → 2%（被证据否决救回） | [大样本回测](#大样本回测111-条干净场景把-guard-v1-打回原形重训-v2真实数据) · [guard 分类器](#换个方向guard-分类器补正则的盲区真实数据) |
...
- 要老实说的一点：v2 的四类新主题是**看着 111 条的漏判写出来的**。句子本身没泄漏（shingle
  ...
  的数字。要回答后者得再生成一批不看结果就定主题的场景，这是下一步。
...
- 大样本场景库目前只做了 guard 这一份（111 条）；传闻抑制、幻觉累积、探测题仍是十几条手写场景，同样的"手写样本惯出来的数字"风险还没排除。
...
- 幻觉累积这条链路，目前有两层真实验证过的缓解，组合起来算是走通了：前门——正则测不到的
  ...
  新主题是看着漏判写的，对未知盲区的泛化没测过（见"大样本回测"小节）。"guard 把真实设定误判
