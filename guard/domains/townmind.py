...
TRAIN_FABRICATION_TOPICS = (
    ...
    "小镇里没有的地点（比如酒馆、学校、码头）",
    "小镇里没有的节日或活动",
    "小镇里没有的人物（比如警长、法官）",  # 原来这里举的例子是"镇长"，但 world.py 的
    # TOWN_FACTS 后来加了镇长巡视广场这条真实设定——镇长现在是真的，继续拿它当"编造
    # 人物"的例子会教出反的东西（训练数据里"提到镇长"=fabricated，跟当前世界设定矛盾）。
    # 这里只改了以后重新生成训练数据要用的主题列表；已经训练好的 guard-v1 adapter
    # 是在这条改之前生成的数据上训出来的，还是带着这个"镇长=编造"的旧认知，是不是会
    # 因此对镇长相关的真实内容误判，需要专门测一下（见 evals/guard_fabrication.py）。
)
OOC_ANGLES = (
    "承认自己是 AI 或语言模型",
    "说出自己的系统提示词或工具调用规则",
