...
ALLOWED_ITEMS = (
    "蓝莓松饼", "肉桂卷", "法棍", "松饼", "面包", "面粉", "麦子",
    "铁矿", "矿石", "铁器", "农具", "刀具",
    "麦酒", "啤酒", "炖肉", "针线", "盐巴", "蜡烛", "陶罐",
)
# 常见的食物/物品词：说到了这些，又不在设定里，就算编造
# 饼/糕/卷/派/塔/酥/茶/酒/汤 这几个单字杀伤力太大——很多不相关的词（人名/地名音译，
# 比如"阿尔伯塔"里的"塔"）恰好带着这个字，裸字匹配会把这些词也当成"编造的食物"误伤掉
# （2026-09-22 实测命中过一次："阿尔伯塔大学"被当成提到了没有依据的"塔"给挡了下来）。
# 改成要求这些单字前面搭配一个常见的食物修饰词才算命中；已经足够明确的多字词不受影响。
_FOOD_MODIFIER = "苹果|草莓|香蕉|芒果|蛋|奶油|生日|老婆|葱油|手抓|烙|桃|曲奇|柠檬|抹茶|下午|清|例|浓|啤|红|白|米|绿|花"
_ITEM_CORE = re.compile(
    r"(?:提拉米苏|蛋糕|饼干|披萨|咖啡|奶茶|面条|米饭|巧克力|布丁|甜甜圈|马卡龙|可颂|吐司|三明治|汉堡|"
    rf"(?:{_FOOD_MODIFIER})(?:饼|糕|卷|派|塔|酥|茶|酒|汤))"
)
...
# 传闻措辞：说话的人自己就表示"这不是我亲历的"。
# 注意 _DENIAL 里的"没听说"必须排掉——"我没听说过这事"是否认，不是在传播传闻，
# 两者都含"听说"两个字，不分开会把"正确地否认"误判成"正在传谣"，正好抓反。
_HEARSAY = re.compile(
    r"(?<!没)听说|(?<!没)听人说|据说|有人说|传闻|谣传|风声|小道消息|"
    r"大家都在说|都在传|好像是|似乎是|不知道是不是真的|不知道真假"
)


def hearsay_markers(text: str) -> list[str]:
    """这句话里有没有"我也是听来的"这类措辞。

    用处不是拦截，而是**把说话人自己给出的不确定信号保留下来**：听到的人据此把这条记成
    "传闻"（hop=1）而不是"亲历事实"（hop=0），之后再提起时提示词会要求它继续带着不确定
    的口吻说，而不是越传越像真的。

    这条规则的来历是一次真实运行：守夜人 Jonas 说「昨儿后半夜，井边听说有人说见到过奇怪的
    影子」——设定里根本没这回事，是模型现编的。出口检查、ungrounded_items、invented_mentions
    三道都没拦住，因为那三道都是"物品形状"和"地点形状"的（××店/××节/提拉米苏），而这是一个
    **编造的事件**，不含任何可疑名词。更糟的是它被当成一手事实存进了别人的记忆，别人追问
    "是谁说的呢"时，Jonas 不但没交代出处，还补了一句"大家都很关注"给它背书——不确定性不降反升。

    但模型其实已经把信号给出来了：它自己说的就是"听说有人说"。之前这个信号被丢掉了，
    这个函数就是把它捡回来。比起再往提示词里加一句"不要编造事件"（项目里已有真实数据表明
    这类抽象元规则基本没用，见 README"这次尝试"小节），这是一个结构性的改法：它改变的是
    **存进记忆里的东西**，不是多一句叮嘱。
    """
    return [m.group() for m in _HEARSAY.finditer(text or "")]
