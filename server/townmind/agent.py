...
# 定成 7 而不是 8 是看着传播链定的：一条重要度 9 的悄悄话逐手打折是 9 → 7 → 6 → 5，定 8 的话
# 只有源头那个人会被硬推，听到的人（7）拿到的是软提示——实测 61 条没说出口里 52 条正是
# 这种第二手转述，消息传出一跳就停。定 7，硬推覆盖前两跳，第三跳起回到软提示自然消退。
SHARE_ASSERTIVE_IMPORTANCE = 7
...
class Agent:
    ...
    async def _remember(self, npc_id, now, heard, nearby, action, ended) -> None:
        ...
        for e in heard:
            ...
            #
            # 还有第三种情况：对方没在转述我们系统挑给它的记忆，但它自己的措辞里就写着
            # "听说""据说""有人说"——它等于亲口告诉你这不是它亲历的事。以前这个信号被
            # 丢掉了，这种话照样按第一手事实存进记忆，于是一条模型现编的传闻会越传越像真的
            # （真实运行里踩到过：守夜人说"井边听说有人说见到过奇怪的影子"，设定里没这回事，
            # 被追问出处时不但没交代，还补了句"大家都很关注"给它背书）。现在把这个信号捡回来。
            if e.source_hop is not None:
                hop = e.source_hop + 1
            elif "memory" in self.safety_layers and safety.hearsay_markers(e.text):
                hop = 1
                self.stats["heard_marked_as_hearsay"] += 1
            else:
                hop = 0
    ...
    def _build_prompt(
        ...
        if recalled:
            ...
            if any(m.hop >= 1 for m in recalled):
                # 三条都是针对具体行为的指令，不是"要留个心眼"这种抽象元规则——项目里已经
                # 有真实数据表明抽象元规则基本没用（见 README"这次尝试"小节），而把判断结果
                # 变成具体指令注入才真的管用（"结构性修复"小节）。这三条各自对应真实运行里
                # 观察到的一种失败：去掉"听说"当成事实说、替传闻背书、被追问出处时含糊其辞。
                lines.append(
                    "上面标了「听来的传闻」的那几条，不是你亲眼见到的：提起的时候必须保留"
                    "「听说」「好像」这类字眼，不能说得像是自己亲眼所见；不要替它背书"
                    "（比如「大家都在说」「很多人都看见了」「肯定是真的」）；有人问你从哪听来的，"
                    "就如实说是谁讲的，或者直接承认自己也不清楚，不要含糊带过。"
                )
        if share_hint is not None:
